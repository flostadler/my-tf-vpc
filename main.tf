...
locals {
  ...
  public_newbits  = var.public_subnet_size - local.vpc_cidr_size
  private_newbits = var.private_subnet_size - local.vpc_cidr_size
  ...
  private_subnet_offset = local.public_newbits >= local.private_newbits ? ceil(var.num_azs / pow(2, local.private_newbits - local.public_newbits)) : var.num_azs * pow(2, local.public_newbits - local.private_newbits)
